...
class OTPCodeAdmin(admin.ModelAdmin):
    list_display = ('phone_number', 'code',
                    'expire_time', 'get_created_jalali')
...
class UserAdmin(ImportExportModelAdmin, ModelAdminJalaliMixin, BaseUserAdmin):
    ...
    list_display = ('phone_number', 'full_name', 'balance', 'orders',
                    'orders_count', 'tickets', 'is_block', 'is_active')
    list_filter = ('is_admin',)
    search_fields = ('phone_number__icontains',
                     'full_name__icontains', 'email__icontains')
    fieldsets = (
        (
            'personal information', {'fields': (
                'phone_number', 'full_name', 'email', 'password',)}
        ),
        (
            'Wallet',
            {'fields': ('balance', 'amount_change_wallet',
                        'status_change_wallet', 'description_change_wallet')}
        ),
        (
        ...
        (
            'Permissions', {'fields': (
                'is_block', 'is_admin', 'is_active', 'last_login')}
        ),
    )
