...
def service_list(request):
    try:
        ...
        services = list(Service.objects.filter(service_type__id=service_type_id,
                        available_for_user=True).order_by('priority').values())
...
def get_description(request):
    try:
        ...
        service_detail = list(
            Service.objects.filter(id=service_id).values())[0]
