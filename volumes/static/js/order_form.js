...
$("#id_service_type").change(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            service_selection.innerHTML = "<option value='' selected>---------</option>"
        ...
        if (!isNaN(product_amount)) {
            if (!isNaN(quantity)) {
                if (parseInt(event.target.value) > max) {
                    ...
                    total_price.textContent = Num2persian(parseInt(product_amount * quantity))
...
if (credit_payment) {
    credit_payment.addEventListener('click', (e) => {
        ...
        let user_balance = parseInt(credit_payment.getAttribute('data-user-balance').replace(',', ''))
...
if (complete_order_payment) {
    complete_order_payment.addEventListener('click', (e) => {
        let user_balance = parseInt(complete_order_payment.getAttribute('data-user-balance'));
        let total_amount = parseInt(complete_order_payment.getAttribute('data-total-amount'));
        let order_id = parseInt(complete_order_payment.getAttribute('data-order-id'));
        let remain_price = 0
        if (total_amount > user_balance) {
            e.preventDefault()
        ...
        let complete_order_button = document.getElementById("complete-order-button")
